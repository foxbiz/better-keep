...
class NoteSyncService {
  ...
  Future<void> init() async {
    ...
    if (currentUser != null) {
      ...
      // Only sync if E2EE is ready
      final e2eeStatus = E2EEService.instance.status.value;
      if (e2eeStatus == E2EEStatus.ready) {
        ...
        AppLogger.log(
          "[SYNC] Skipping initial sync - E2EE status: $e2eeStatus",
        );
      }
    }
  ...
  void _onE2EEStatusChange() {
    ...
    AppLogger.log("[SYNC] E2EE status changed to $status");
    ...
    if (status == E2EEStatus.ready) {
      ...
      AppLogger.log("[SYNC] E2EE ready, triggering full sync");
      AppState.lastSynced = null; // Force full sync
      refresh();
      _startRemoteListener();
  ...
  Future<void> sync([bool now = false]) async {
    ...
    if (!_canPushSync) {
      AppLogger.log(
        "[SYNC] Skipping sync request - push sync not allowed",
      );
      return;
    }
  ...
  Future<void> _processCachedSyncs() async {
    ...
    AppLogger.log(
      "[SYNC] PROCESS START: $totalCount cached syncs to process",
    );
    ...
    for (final pendingSync in pendingSyncs) {
      ...
      try {
        ...
        if (success) {
          ...
          syncProgress.value = (syncedCount, totalCount);
          AppLogger.log(
            "[SYNC] PROCESS: Note $localId synced successfully",
          );
        } else {
    ...
    try {
      ...
      if (looksEncrypted) {
        ...
        try {
          ...
          AppLogger.log(
            "Failed to decrypt attachment: $e - will retry later",
          );
      ...
      AppLogger.log(
        "Failed to download from $src: ${e.code} ${e.message}",
      );
    ...
    try {
      ...
      try {
        ...
        if (looksEncrypted) {
          if (!e2ee.isReady) {
            AppLogger.log(
              "Cannot decrypt redownloaded file - E2EE not ready",
            );
            return null;
          }
        ...
        AppLogger.log(
          "Failed to redownload file: ${e.code} ${e.message}",
        );
        if (e.code == 'object-not-found') {
