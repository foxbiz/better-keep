...
class WebFileSystem implements FileSystem {
  ...
  Future<void> _listRecursiveHelper(
    ...
    for (var entry in entries) {
      // Sanitize entry name - remove any leading/trailing slashes
      // and skip entries that contain slashes (corrupted/invalid names)
      entry = entry.trim();
      if (entry.startsWith('/')) {
        entry = entry.substring(1);
      }
      if (entry.isEmpty || entry.contains('/')) {
        // Skip corrupted entries with paths as names
        result.add({
          'path': '$directory/$entry',
          'type': 'corrupted',
          'message': 'Entry name contains invalid characters',
        });
        continue;
      }
...
class _IndexedDbBackend implements _FileSystemBackend {
  ...
  Future<List<String>> list(String directory) async {
    ...
    await store.openCursor(autoAdvance: true).forEach((cursor) {
      if (cursor != null) {
        allKeys.add(cursor.key.toString());
      }
    });

    await txn.completed;

    // IndexedDB stores flat key-value pairs with full paths as keys.
    // We need to filter to entries directly under the given directory
    // and return only the immediate child name (not full path).
    final normalizedDir = directory == '/' ? '' : directory;
