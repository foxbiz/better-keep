...
class _NerdStatsPageState extends State<NerdStatsPage> {
        ...
        if (_loadingOpfs)
          ...
          ..._opfsFiles.map(
            (file) => ListTile(
              ...
              onTap: file['type'] == 'file'
                  ? () => _showFileContent(file['path'] as String)
                  : null,
  ...
  Future<void> _showFileContent(String filePath) async {
    if (!mounted) return;
    ...
    try {
      ...
      if (isImage) {
        ...
        _showImageDialog(filePath, bytes);
      } else if (isAudio) {
        final bytes = await fs
            .readBytes(filePath)
            .timeout(
        ...
        _showAudioDialog(filePath, bytes);
      } else {
        final content = await fs
            .readString(filePath)
            .timeout(
              const Duration(seconds: 10),
              onTimeout: () => 'Error: Timeout reading file',
            );

        if (!mounted) return;
        Navigator.of(context).pop(); // Dismiss loading dialog

        _showTextDialog(filePath, content);
      }
    } catch (e) {
      if (!mounted) return;
      Navigator.of(context).pop(); // Dismiss loading dialog
      ...
      showDialog(
        ...
        builder: (context) => AlertDialog(
          ...
          content: SelectableText('Failed to read file:\n$e'),
          actions: [
            TextButton(
  ...
  void _showTextDialog(String filePath, String content) {
    showDialog(
      context: context,
      builder: (context) => AlertDialog(
        title: Text(
          ...
          TextButton(
            onPressed: () {
              SharePlus.instance.share(
                ShareParams(text: content, title: filePath.split('/').last),
              );
            },
  ...
  void _showImageDialog(String filePath, Uint8List bytes) {
    if (bytes.isEmpty) {
      _showTextDialog(filePath, 'Error: Failed to load image');
      return;
    }
          ...
          child: InteractiveViewer(
            child: Image.memory(
              ...
              errorBuilder: (context, error, stackTrace) =>
                  Center(child: Text('Failed to display image: $error')),
            ),
          ),
        ),
        actions: [
  ...
  void _showAudioDialog(String filePath, Uint8List bytes) {
    if (bytes.isEmpty) {
      _showTextDialog(filePath, 'Error: Failed to load audio');
      return;
    }
    ...
    showDialog(
      context: context,
      builder: (context) =>
          _AudioPlayerDialog(filePath: filePath, audioBytes: bytes),
    );
  }
...
/// Audio player dialog for playing audio files from OPFS
class _AudioPlayerDialog extends StatefulWidget {
  final String filePath;
  final Uint8List audioBytes;

  const _AudioPlayerDialog({required this.filePath, required this.audioBytes});
