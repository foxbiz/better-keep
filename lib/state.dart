...
class AppState {
  ...
  static Future<void> init() async {
    ...
    // Load theme settings
    final followSystemTheme = prefs.getBool("follow_system_theme") ?? false;
    final darkThemeId =
        prefs.getString("dark_theme_id") ?? ThemeRegistry.defaultDarkThemeId;
    final lightThemeId =
        prefs.getString("light_theme_id") ?? ThemeRegistry.defaultLightThemeId;

    // Migrate from old theme_name if exists
    String themeId;
    final oldThemeName = prefs.getString("theme_name");
    if (oldThemeName != null) {
      // Migrate old settings
      themeId = oldThemeName == "light"
          ? ThemeRegistry.defaultLightThemeId
          : ThemeRegistry.defaultDarkThemeId;
      prefs.remove("theme_name");
      prefs.setString("theme_id", themeId);
      prefs.setString("dark_theme_id", darkThemeId);
      prefs.setString("light_theme_id", lightThemeId);
    } else {
      themeId = prefs.getString("theme_id") ?? ThemeRegistry.defaultDarkThemeId;
    }
    ...
    var alarmSound = prefs.getString("alarm_sound") ?? defaultAlarmSound;
    final lastSyncedAtString = prefs.getString("last_synced_at");
    final lastLabelSyncedAtString = prefs.getString("last_label_synced_at");
    final recentColors =
        prefs
    ...
    if (alarmSound.startsWith("lib/")) {
      ...
      prefs.setString("alarm_sound", alarmSound);
    }
    ...
    // Load time settings
    final morningHour = prefs.getInt("morning_time_hour") ?? 6;
    final morningMinute = prefs.getInt("morning_time_minute") ?? 0;
    final afternoonHour = prefs.getInt("afternoon_time_hour") ?? 12;
    final afternoonMinute = prefs.getInt("afternoon_time_minute") ?? 0;
    final eveningHour = prefs.getInt("evening_time_hour") ?? 18;
    final eveningMinute = prefs.getInt("evening_time_minute") ?? 0;
    ...
    _state["show_sync_progress"] = prefs.getBool("show_sync_progress") ?? true;
  }
