...
class AppState {
  ...
  static Future<void> init({SharedPreferences? prefs}) async {
    ...
    // Load sketch tool preference
    final sketchToolName = prefsInstance.getString("sketch_tool");
    if (sketchToolName != null) {
      _state["sketch_tool"] = SketchTool.values.firstWhere(
    ...
    final penColorValue = prefsInstance.getInt("sketch_pen_color");
    if (penColorValue != null) {
  ...
  static SketchTool get sketchTool {
    return _state["sketch_tool"] as SketchTool;
  }

  static set sketchTool(SketchTool value) {
    set("sketch_tool", value);
    _persistToPrefs((p) async => p.setString("sketch_tool", value.name));
  }

  static double get sketchPenSize {
    return _state["sketch_pen_size"] as double;
  }

  static set sketchPenSize(double value) {
    set("sketch_pen_size", value);
  ...
  static Color get sketchPenColor {
    return _state["sketch_pen_color"] as Color;
  }

  static set sketchPenColor(Color value) {
    set("sketch_pen_color", value);
    _persistToPrefs(
      (p) async => p.setInt("sketch_pen_color", value.toARGB32()),
    );
  }
