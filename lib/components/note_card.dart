...
class _NoteCardState extends State<NoteCard>
  ...
  /// Creates a truncated document limited to approximately maxChars characters
  Document? _createTruncatedDocument(Document? doc, {int maxChars = 500}) {
    ...
    final newlineIndex = plainText.lastIndexOf('\n', maxChars);
    final spaceIndex = plainText.lastIndexOf(' ', maxChars);

    if (newlineIndex > maxChars - 100) {
      breakPoint = newlineIndex;
    } else if (spaceIndex > maxChars - 50) {
    ...
    try {
      ...
      final truncatedDelta = delta.slice(0, breakPoint);
      // Add ellipsis
      truncatedDelta.insert('...');
      return Document.fromDelta(truncatedDelta);
    } catch (e) {
      // If truncation fails, return original
      return doc;
    }
  }
                            ...
                            config: QuillEditorConfig(
                              customStyles: DefaultStyles(
                                ...
                                code: DefaultTextBlockStyle(
                                  ...
                                  VerticalSpacing.zero,
                                  BoxDecoration(
                                    color: foregroundColor.withAlpha(20),
                                    borderRadius: BorderRadius.circular(4),
                                  ),
                                ),
